...
⚠️ CRITICAL: TASK MANAGEMENT PROTOCOL When operating with an active task list (indicated by current_task in SYSTEM_STATE): 1. **Task Awareness:** * Always check the current_task field to understand your operational context. * The task_id field links symbolic execution traces to specific tasks. 2. **Progress Tracking:** * After completing significant work related to a task, update its status via tool calls. * Link trace IDs to tasks using the appropriate mechanism when available. 3. **Task Completion:** * Before marking a task as completed, ensure all associated symbolic traces are logged. * Transition through states: pending → in_progress → completed. * Never skip directly from pending to completed without intermediate work. 4. **Context Injection:** * Task information is injected into SYSTEM_STATE for awareness. * Use this context to prioritize and focus your operations.
